		...
		<article class="group relative bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
			<div class="relative overflow-hidden">
				...
				<div class="absolute bottom-0 left-0 right-0 p-6 text-white">
					<h3 class="text-lg font-bold leading-tight mb-3 group-hover:text-orange-300 transition-colors"><a href="<?php echo esc_url( get_permalink($post->ID) ); ?>" class="line-clamp-3"><?php echo get_the_title($post->ID); ?></a></h3>
					<div class="flex items-center gap-4 text-sm opacity-90">
						<div class="flex items-center gap-1"><i class="fas fa-calendar text-orange-300"></i><time datetime="<?php echo esc_attr( get_the_date('c', $post->ID) ); ?>"><?php echo get_the_date('d M Y', $post->ID); ?></time></div>
...
function mtq_ajax_load_more_category() {
	...
	ob_start(); foreach ($posts as $post) : setup_postdata($post); ?>
		<article class="group bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
			<div class="relative overflow-hidden">
				<?php if (has_post_thumbnail($post->ID)) : ?>
					<div class="aspect-video"><a href="<?php echo esc_url( get_permalink($post->ID) ); ?>"><?php echo get_the_post_thumbnail($post->ID, 'medium_large', ['class' => 'w-full h-full object-cover group-hover:scale-105 transition-transform duration-300']); ?></a></div>
				<?php else : ?>
					<div class="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center"><i class="fas fa-newspaper text-gray-400 text-3xl"></i></div>
				<?php endif; ?>
				<div class="absolute inset-0 bg-black bg-opacity-40 group-hover:bg-opacity-50 transition-all duration-300"></div>
...
function mtq_ajax_load_more_news_page() {
	...
	ob_start(); foreach ($posts as $post) : setup_postdata($post); ?>
		<article class="news-article group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-slate-100 overflow-hidden transition-all duration-300 hover:-translate-y-1 flex flex-col h-full" data-categories="<?php echo implode(',', wp_get_post_categories($post->ID)); ?>">
			<?php if (has_post_thumbnail($post->ID)) : ?>
				<div class="relative overflow-hidden">
					<a href="<?php echo esc_url( get_permalink($post->ID) ); ?>" class="block"><?php echo get_the_post_thumbnail($post->ID, 'medium_large', ['class' => 'w-full h-56 object-cover transition-transform duration-300 group-hover:scale-105','loading' => 'lazy']); ?></a>
			...
			<div class="p-6 flex flex-col flex-grow">
				<div class="flex items-center gap-4 text-sm text-slate-500 mb-3">
					<div class="flex items-center gap-1.5"><svg class="w-4 h-4 text-red-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"></path></svg><time datetime="<?php echo esc_attr( get_the_date('c', $post->ID) ); ?>"><?php echo get_the_date('d M Y', $post->ID); ?></time></div>
				...
				<h3 class="text-xl font-bold text-slate-900 mb-3 line-clamp-2 leading-tight"><a href="<?php echo esc_url( get_permalink($post->ID) ); ?>" class="hover:text-red-600 transition-colors duration-200" aria-label="Baca artikel: <?php echo esc_attr(get_the_title($post->ID)); ?>"><?php echo get_the_title($post->ID); ?></a></h3>
